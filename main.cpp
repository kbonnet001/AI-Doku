...
int main()
{
    ...
    BoutonClose boutonClose(Vector2f(35.f, 743.f));
    Grille grilleJeu=Grille();
    BoutonEcrire essaiBouton = BoutonEcrire(0, Vector2f(0.f, 0.f));
    ...
    while (window.isOpen())
    {
        ...
        if (event.type == Event::MouseButtonReleased && event.mouseButton.button == Mouse::Left)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                {
                    grilleJeu.avoirBoutonEcrire(i).actionBouton(window, mousePosition);
                }
            }
        }
